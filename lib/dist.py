...
class GumbelSoftmax_catY(nn.Module):
    def __init__(self, nClass):
        super(GumbelSoftmax_catY, self).__init__()
        self.K = nClass
    ...
    def sample_test(self, alpha):
        _, max_alpha = torch.max(alpha, dim=1)
        one_hot_samples = torch.zeros(alpha.size())
        one_hot_samples = one_hot_samples.scatter_(1, max_alpha.view(-1, 1).data.cpu(), 1).type_as(alpha)
...
class Normal(nn.Module):
    ...
    def __init__(self, mu=0, sigma=1, dimSpatial=False):
    ...
    def _check_inputs(self, size, mu_logsigma):
        if size is None and mu_logsigma is None:
            ...
            if self.dimSpatial == True:
                outCh = mu_logsigma.size(1) / 2
                mu = mu_logsigma[:, :outCh, :, :]
                logsigma = mu_logsigma[:, outCh:, :, :]
            else:
